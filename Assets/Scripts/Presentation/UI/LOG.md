...
## 2026-08-20 — 게임오버 화면 (코어 파괴)

코어가 부서져도 화면에 아무 일도 일어나지 않았다. `BattleManager.GameOver` 이벤트는 2026-08-12부터
있었지만 **구독자가 프로젝트 전체에 0개**여서(전수 확인), 플레이어는 코어 없는 세계에 그대로 남았다 —
시간은 흐르고 스폰만 조용히 멈추고, 나갈 방법도 되돌릴 방법도 없었다.

만든 것은 화면 하나뿐이다. 복구·타이틀 복귀·슬롯 목록은 이미 있는 것을 부른다
(`LoadMostRecent` · `ReturnToTitle` · `SaveLoadPanelView`).

### 닫히지 않는 팝업

`UITKPopup`을 그대로 상속하되 **ESC·✕·"계속하기"가 없다.** 코어가 없는 세계로 돌아가 봐야 할 수
있는 일이 없으므로, 닫을 수 있게 만드는 것은 선택지를 주는 게 아니라 막다른 곳에 두는 것이다.

- `OnInput`이 Cancel을 **삼킨다**(`return true`). 소비가 핵심 — 흘려보내면 뒤쪽
  `SystemUIManager`(우선순위 Fallback)가 받아 게임오버 위에 일시정지 메뉴가 열린다
- `Close()`는 no-op으로 덮었다. 외부에서 불러도 닫히지 않는다
- 열릴 때 `PauseMenuView.CloseIfOpen()` — 코어가 터지고 화면이 뜨기까지의 1.5초 사이에 일시정지를
  열어뒀을 수 있다. 그 창의 "계속하기"는 이제 갈 곳이 없다

### 띄우는 쪽은 패널이 아니다 — `GameOverPresenter`

패널 호스트는 프리팹에서 꺼져 있어 `OnEnable`이 돌지 않는다. 꺼진 오브젝트는 자기가 열려야 할
순간을 알 수 없으므로, `GameUI_UITK` **루트**(항상 켜짐)에 붙는 프레젠터가 대신 듣는다.
`BattleManager`는 씬마다 새로 생기고 신호도 인스턴스 이벤트라, 붙을 때까지 매 프레임 다시 찾는다
(HUD가 코어·플레이어 엔티티를 대하는 방식과 같은 규칙).

바인딩 직후 `IsGameOver`를 한 번 검사한다 — `RestoreGameOver`는 사망 연출이 로드할 때마다 다시
돌지 않도록 이벤트를 재발화하지 않으므로, 이게 없으면 **게임오버 상태로 저장된 세이브를 열었을 때
UI 없는 죽은 세계**가 된다.

### 조용히 위험했던 것 — 죽은 상태가 세이브를 덮어쓴다

`ProgressSaveModule`은 `gameOver` 플래그를 저장하고, `autoSaveOnQuit`은 기본 켜짐이며,
종료와 타이틀 복귀가 둘 다 `AutoSave()`를 부른다. 손대지 않았다면 **게임오버 화면에서 나가거나
게임을 끄는 순간 자동 저장이 죽은 세계로 덮어씌워지고, 타이틀의 "이어하기"가 그리로 들어갔다.**

`SaveManager.ShouldSuppressAutoSave()` 한 곳으로 막는다(게임오버 중 + 복원 중). 여기 하나면
아침·밤 자동 저장, 종료 시 저장, 타이틀 복귀 시 저장 네 경로가 전부 덮인다. 화면의 타이틀 버튼도
`saveFirst: false`로 부른다 — 둘 중 하나가 사라져도 다른 하나가 남는다.
수동 저장에는 가드를 두지 않았다. 게임오버 중에는 도달할 수 없고, 수동 저장을 조용히 실패시키는
것은 더 나쁜 계약이다.

### 밤 시작 자동 저장 (사용자 결정)

밤은 코어가 부서질 수 있는 유일한 시간이다. 아침 저장만 있으면 재시도가 **밤을 통째로 다시 사는
일**이 되고, 첫날 밤에 죽으면 돌아갈 곳이 아예 없다. `SaveSystemConfig.autoSaveOnNightStart`(기본 켬)
+ `SaveManager.AutoSaveOnNightStart()` + `GameManager`가 `Cycle.NightStarted`를 구독.
자동 슬롯이 2개라 이제 **아침 1개 + 밤 1개**가 순환 유지된다.
`DayCycle.RestoreState`가 이벤트를 발화하지 않는 것을 확인했다 — 로드할 때 되돌아 도는 일은 없다.

### 스타일

`saveload.uss`(타이틀·일시정지·슬롯)에 넷째 화면으로 합류. 세로 메뉴는 `.pause-status`/`.pause-btn`을
그대로 쓰고, 새로 적은 것은 문구 세 줄과 폭(480px — 일시정지 340보다 넓다, 버튼만 있는 창이 아니라
문장이 들어간다)뿐이다. 태그 색 변형 `.ui-panel__tag--danger`는 `--crystal` 옆에 추가.

**함정: UXML 주석에 `--`를 쓸 수 없다.** 주석에 "`--danger`로 멀리 놓인다"라고 적었다가
XML 파싱이 통째로 실패했다(`An XML comment cannot contain '--'`). USS 클래스명을 주석에서
언급할 때 반드시 걸린다.

### 검증 (플레이 실측 · MainScene)

코어 파괴 → 1.5초 뒤 화면 표시 · `timeScale=0` · 패널 480×562 ·
헤드라인 `RGBA(1.000,0.365,0.451)`(=`--danger`)/30px · 스크림 `rgba(6,10,20,.55)` — USS 적용 확인.
ESC 눌러도 안 닫히고 일시정지도 안 열림 / `Close()` 호출해도 열린 채 유지.
슬롯 창을 위에 겹쳐 열고 ESC → **슬롯만 닫히고 게임오버는 남음**(depth 우선순위 정상).
세이브 없음 상태(세이브 폴더를 잠시 치움) → 재시도·불러오기 흐려지고 문구 두 줄이 바뀜,
`LoadMostRecent()`도 false. 복구 성공 → `IsGameOver=False`·코어 부활·`timeScale=1`·화면 닫힘.
게임오버 상태로 플레이 종료 → **자동 저장 슬롯 타임스탬프 변화 없음**(가드 실증).
밤 진입 → `[1일차 밤 시작] 자동 저장 완료` → `auto_01`(phase=Night), 그 상태로 코어를 부수니
화면의 "마지막 기록"이 그 밤 저장을 가리킴. 예외 0.

### 알려진 제약 (이 작업 밖)

- **`Assets/Scenes/Test/MapPrototype3.0.unity`가 Build Settings에 없다.** 실제 전투 테스트 맵인데
  등록이 안 돼 있어, 거기서 만든 세이브는 `ResolveScene`이 null을 반환해 **불러오기 자체가 불가능하다**
  (게임오버의 재시도만이 아니라 타이틀의 "이어하기"도 마찬가지). 실측으로 확인했고, 화면은 실패를
  삼키지 않고 안내문을 띄운다. 팀 공유 설정이라 등록은 별도 합의 후에.
