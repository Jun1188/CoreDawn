...
namespace CoreDawn.Sim
{
    ...
    /// 섹션 = items · recipes · effects · entities · guns · tutorial · sounds · materials(재질 — 표현 전용) · wave(규칙 하나 — 밤 웨이브 점수식) · dayCycle(주야 시계 하나) · sfx(공용 소리 자리 — 뷰가 Raw로 읽는다).
    ...
    public sealed class SimDatabase
    {
        ...
        {
            ("items", "item"), ("recipes", "recipe"), ("effects", "effect"), ("entities", "entity"), ("guns", "gun"), ("tutorial", "tutorial"), ("sounds", "sound"), ("materials", "material"),
        };
        ...
        readonly Dictionary<string, MaterialDef> materials = new Dictionary<string, MaterialDef>();
        ...
        public IReadOnlyDictionary<string, MaterialDef> Materials => materials;
        ...
        public static SimDatabase Load(string json, string pack, bool strict = true)
        {
            ...
            db.LoadSection(root, "materials", "material", db.materials, serializer);
            db.Raw = root;
        ...
        public MaterialDef Material(string id) => materials.TryGetValue(id, out var d) ? d : null;
    }
}
