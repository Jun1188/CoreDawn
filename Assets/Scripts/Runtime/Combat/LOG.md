...
## 2026-08-13 (2) — 발사 진입점 단일화 + 히트스캔 트레이서 (판정·연출 분리)

### 원칙 (사용자 제기 — "통일성·발사체의 이펙트·의존성을 생각하면 전부 소환이어야 하지 않나")
반은 수용, 한 축은 분리했다:

- **진입점은 하나**: `ProjectileSystem.Fire(origin, direction, shot)` — shot에 Mode(발사기
  소유)·Prefab(탄약 소유)이 실린다. 총·타워의 `if (Hitscan) ... else ...` 분기 전부 삭제.
  발사기는 이제 전달 방식을 모른다 — 스펙만 만들면 분기는 시스템에서 끝난다.
- **인스턴스는 전 모드 소환(연출)**: 히트스캔은 판정 직후 같은 탄 프리팹을
  **트레이서**(효과 없음·TargetMask 0)로 날린다 — 스윕이 벽·대상에서 멈추므로 궤적이
  판정과 시각적으로 일치한다(FPS 고전 기법). 무형이던 샷건·저격·레이저가 보이게 됐다.
  프리팹이 없으면 연출만 생략 — 판정은 그대로(헤드리스 안전).
- **판정은 인스턴스에 태우지 않는다**: 히트스캔·오라 판정을 소환된 오브젝트의 Update로
  미루면 프레임 지연이 생기고 판정이 풀 수명에 결합된다. 판정은 모드의 물리대로 —
  즉시(히트스캔·오라) / 비행 스윕(투사체). 인스턴스는 눈을 위해서만 존재한다.

### 검증 (플레이 실측, 같은 프레임 관찰)
히트스캔 발사 직후 동일 프레임에서 HP 100→70(-30 정확히 1회 — 판정 즉시·트레이서 중복
없음), 활성 탄 0→1(트레이서 소환), 감속 부가효과 0.5 적용.

---

## 2026-08-14 — 탄약 실소비·관통·최소 사거리

### ① 총 탄약 실소비 (합의 "실소비로 가되 단계적으로"의 1단계)
포탑이 벨트 보급 탄을 소비하듯 총도 인벤토리의 탄 아이템을 소비한다:

- `Gun.CurrentAmmoItem` — 지금 장전된 탄종(런타임 상태). 발사 스펙(효과·탄도)의 출처가
  gunData.ammo(고정)에서 이것으로 바뀌었다. 시작은 gunData.ammo.
- **재장전 = 인벤토리 소모**: PlayerInventoryHolder(메인 가방 → 핫바 순)에서 실제로
  꺼낸다. 모자라면 부분 장전, 하나도 없으면 재장전이 시작조차 안 된다.
  실소비 세계에서는 빈 탄창으로 시작한다 — 첫 발도 인벤토리에서 나와야 한다.
- **탄종 전환** `TrySwitchAmmo()`: `GunData.ammoFilter`(신설 — 포탑과 같은 개념, json
  ammoFilter) 안에서 인벤토리에 있는 다음 탄종으로 돈다. 장전돼 있던 탄은 인벤토리로
  반환(전환이 탄을 증발시키면 안 된다) 후 자동 재장전. 입력 바인딩·HUD는 UI 후속
  (`ReserveAmmo` 프로퍼티만 게시).
- 인벤토리가 없는 씬(테스트)은 추상 탄창 폴백 — 무한 보급, 헤드리스 안전.

### ③ 관통 — 탄의 성질
`AmmoModuleSO.pierce`(추가 관통 대상 수) → `ProjectileShot.Pierce`. Hitscan이 Pierce>0이면
경로의 모든 히트를 거리순으로 밟아 (1+Pierce)명까지 뚫는다 — 단 대상 레이어가 아닌
콜라이더(벽·건물)에서 빔이 멈춘다. 에너지 셀 탄 pierce 4 — 에너지 소총과 레이저 타워가
탄 공유로 동시에 관통을 얻는다. (투사체 관통은 미구현 — 관통탄을 쓰는 Projectile
발사기가 아직 없다. 트레이서가 첫 대상에서 멈추는 연출 격차도 알려진 것.)

### ④ 최소 사거리 — 박격포의 사각
`TowerDataSO.minRange` + `SensorComponent.GetClosestTarget(range, minRange)` — 사각 안의
적은 조준 대상에서 빠진다(더 먼 유효 표적이 있으면 그쪽을 쏜다). 박격포 minRange 5.

### 검증 (플레이 실측)
- 관통: 일렬 3마리가 히트스캔 한 발에 전원 -30 (100→70 70 70).
...
## 2026-08-14 (3) — 발사 정합(조준선·총구 합류)·이펙트·자기 명중 차단

### 발사 축을 조준선으로 (사용자 지적 3건의 해소)
총구 축으로 쏘면 ① 비행 중 충돌이 조준선 밖에서 판정되고 ② 근접 표적에서 탄이 옆으로
날며 ③ 중력탄의 포물선이 크로스헤어와 어긋난다. 그래서:
- **판정·직진은 카메라(조준선) 기준** — Gun.Fire의 origin/forward가 카메라다.
- **탄의 몸은 총구에서 태어나** 조준선의 합류점까지 날아간 뒤 조준선 방향으로 꺾인다
  (`ProjectileShot.Muzzle`·`Bullet.SetJoin`). 카메라에서 태어나 화면을 가리지 않는다.
- 합류점 = **총구를 조준선에 투영한 지점 + 1m**. 원점(카메라) 기준으로 잡으면 총구가
  카메라보다 앞에 있을 때 합류점이 총구 뒤가 되어 탄이 역주행한다.
- 탄두는 스폰부터 최종 진행 방향을 본다 — 합류점을 보면 비스듬히 나는 것처럼 보인다.
- 히트스캔 트레이서 속도는 300 고정(연출값), 수명은 비행시간 비례.

### 탄이 자기 자신에 맞던 버그 (사용자가 원인 특정)
스피어캐스트는 시작점에서 겹친 콜라이더를 거리 0으로 보고한다 → 탄의 콜라이더가
트리거가 아니면 **태어난 자리(총구)에서 즉시 착탄**. 방향과 무관하게 화면상 같은 자리에서
터지는 것처럼 보였고, 히트스캔은 트레이서가 스윕을 하지 않아 멀쩡해 보였다.
- `ProjectileSystem.SweepLayers` — 기본 레이캐스트 레이어에서 **Bullet 레이어 제외**(정본).
- `Bullet.OnValidate` — 프리팹의 레이어(Bullet)와 콜라이더 트리거를 자동 보정(이중 방어).
- 콜라이더는 이제 판정에서 완전히 빠지고 탄 굵기(스윕 반경)의 정보원일 뿐이다.

### 연출 (탄약이 연출의 주인)
- `AmmoModuleSO.muzzleFlashPrefab`·`hitEffectPrefab` — 같은 탄이면 총이든 타워든 같은 연출.
  총구 화염은 방아쇠당 1회 총구에 부착, 착탄 이펙트는 벽·관통 대상마다·폭발 중심에서.
- **풀링**(`PooledEffect`) — 탄과 같은 풀. 파티클이 끝나면 자동 반환, 반환 시 부모를 풀
  루트로 되돌린다(총구에 붙은 채 반환되면 무기와 함께 파괴돼 죽은 참조가 남는다).
- **파티클 되살리기** — playOnAwake는 최초 1회뿐이라, 풀 재사용 시 `Clear+Play`를 해주지
  않으면 두 번째 발사부터 탄이 날아가도 보이지 않는다. 트레일도 Clear(잔상 순간이동).

### 조준(ADS) 정합
`WeaponMotionManager`가 정렬(ADS)과 절차 모션(반동·스웨이)을 분리 합산한다 — 0.5m 안전
상한은 절차 모션의 폭주 방지용이지, 가늠자를 눈까지 끌어오는 정렬에 적용되면 힙 포즈가
먼 무기의 조준이 중간에 잘린다(실측: 요구 2.26m 중 22%만 적용돼 오차 1.76m).

### 데이터
- `GunData.ammo` 폐지 → **`ammoFilter`의 첫 항목이 기본 탄종**(중복 제거, 사용자 지적).
- 신규 총 5정의 `enemyLayer` 미배선 발견 → Monster로 배선(없으면 쏴도 피해가 없다).

### 검증 (플레이 실측)
총구 출발(카메라에서 2.48m 옆)→합류→15m 표적 명중 / 관통 3인 동시 피해 / 이펙트 풀
5개 생성 후 재사용(인스턴스 증가 없음) / 스윕 마스크에서 Bullet 제외·Monster·Ground 유지.

---

## 2026-08-16 — 버그 정리: 근접 사각지대와 얼어붙은 이펙트

### 총구보다 가까운 것에는 맞지 않았다

탄의 몸은 총구에서 태어난다. 그래서 **총구보다 가까운 표적은 태어난 순간 이미 지나쳐 있다** —
벽에 붙어 쏘거나 몬스터가 코앞에 붙으면 탄이 그것을 통과해 날아갔다. 실측한 총구 거리가
카메라 앞 **2.1~2.9m**라, 사각지대가 생각보다 넓었다.

판정 축은 조준선이라는 원칙 그대로, 합류 구간 `[원점, 합류점]`을 **발사 시점에 미리 레이로
판정**한다(`SpawnAligned`). 맞으면 그 자리에서 착탄하고 탄은 태어나지 않는다.
트레이서(TargetMask 0)는 이미 판정이 끝난 연출이므로 이 선판정을 건너뛴다.

검증: 총구(2.29m)보다 1.14m 앞의 표적 — 발사와 같은 프레임에 HP 100 → 86.

### 쏘지도 않았는데 총구 화염이 터진 이유

총구 화염은 총의 자식으로 붙어 총과 함께 움직인다. 그래서 무기를 집어넣으면
(`SetActive(false)`) 재생 중이던 이펙트가 **무기 자식으로 얼어붙고**, 다시 꺼낼 때
파티클의 `playOnAwake`가 혼자 발동했다.

- **풀 오브젝트에 `playOnAwake`는 재앙이다** — 활성화되는 것만으로 터진다. 첫 재생 때
...
검증: 발사 직후 재생 4 → 집어넣은 뒤 0 → **다시 꺼낸 뒤 0**(쏘지 않았으므로).
