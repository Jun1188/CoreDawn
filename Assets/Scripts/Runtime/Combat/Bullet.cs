...
/// 어떤 속도에서도 명중이 보장되고, 명중 처리(ApplyHit)는 히트스캔과 같은 코드를 쓴다.
///
...
public class Bullet : MonoBehaviour
{
    ...
    private void Update()
    {
        float step = shot.Speed * Time.deltaTime;
        Vector3 pos = transform.position;

        if (ProjectileSystem.TryClosestHit(pos, transform.forward, step, sweepRadius, shooterRoot, out RaycastHit hit))
        {
            ProjectileSystem.ApplyHit(hit.collider, hit.point, shot);
            ReleaseToPool();
            return;
        }

        transform.position = pos + transform.forward * step;

        // 수명·사거리 초과 시 소멸 (Invoke 예약 대신 타이머 — 풀 재사용 시 취소 관리가 필요 없다)
        age += Time.deltaTime;
        if (age >= shot.Lifetime ||
            (transform.position - start).sqrMagnitude >= shot.Range * shot.Range)
        {
            ReleaseToPool();
        }
    }
