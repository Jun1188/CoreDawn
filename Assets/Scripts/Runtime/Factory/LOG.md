...
- **ItemType 재정의(9종)** — Ore/Ingot/Part/RepairPart/Ammo/Weapon/Armor/Placeable/Salvage.
  **정수값 명시**로 기존 에셋의 Ore(0)/Ingot(1)/Component(2)가 자동 승계됨 (enum은 int로 직렬화)
- **ItemLine 신설** — 용도(type)와 직교하는 계통 축(None/Iron/Copper/Crystal/Beast).
  UI 계통색의 근거. `ItemDatabaseSOEditor` 표시명도 함께 갱신
