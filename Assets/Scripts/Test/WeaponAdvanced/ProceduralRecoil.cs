...
/// <summary>
/// 카메라 반동 — 이제 <see cref="ICameraMotionModule"/>이다.
///
...
/// API(<see cref="FireRecoil"/>)는 그대로라 WeaponBase 쪽 호출부는 손대지 않는다.
/// </summary>
public class ProceduralRecoil : MonoBehaviour, ICameraMotionModule
{
    ...
    /// <summary>WeaponBase.ApplyRecoil()이 호출. 화면이 위로 튀고(X), 좌우로 흔들리며(Y), 비틀린다(Z).</summary>
