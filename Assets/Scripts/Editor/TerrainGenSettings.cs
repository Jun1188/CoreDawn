...
public class TerrainGenSettings : ScriptableObject
{
    ...
    // 높이는 쌓지 않고 배치 고도로 만든다: 가장자리 줄만 땅에 서고, 안쪽 바위는 공중에 띄운다.
    // 바닥은 앞줄이 가리므로 보이지 않는다. 쌓으면 이음매가 오레오처럼 줄무늬가 되고,
    // 세로로 늘리면 옆으로 넓은 바위가 콜라캔 판자가 된다 — 둘 다 겪었다.
    // 최저값은 플레이어 점프(1.3m)로 올라설 수 없는 선.
    [Tooltip("능선고 범위 — 칸 크기의 배수. (x=최저, y=최고)")]
    ...
    [Tooltip("높이 노이즈 주파수. 0.11 ≈ 9칸(18m) 파장 — 몇 칸에 걸쳐 솟았다 가라앉는다.")]
    public float cliffRidgeFrequency = 0.11f;

    [Tooltip("벽면 굴곡 주파수. 0.45 ≈ 2칸 파장 — 지그재그 대신 물결.")]
    public float cliffFaceFrequency = 0.45f;

    [Tooltip("가장자리 줄을 땅에 묻는 깊이 — 칸 크기의 배수. 에셋 데모도 Y 최저 -3.4m.")]
    public float cliffBaseSinkCells = 0.3f;

    // 세로는 넉넉히 늘린다(스케일 자유화) — 안쪽은 물러선 만큼 좁아진 발판으로 능선고를
    // 채워야 하고, 가장자리도 낮으면 담장처럼 보인다.
    [Tooltip("가장자리 줄 바위의 세로 배율 범위. (x=최소, y=최대)")]
    public Vector2 cliffStretchEdge = new Vector2(1.15f, 1.75f);
    [Tooltip("안쪽(공중에 뜬) 바위의 세로 배율 범위. (x=최소, y=최대)")]
    public Vector2 cliffStretchInner = new Vector2(1.0f, 1.7f);
