...
namespace CoreDawn.EditorTools
{
    ...
    class GdWaveTab : GdTab
    {
        ...
        public override void Build(VisualElement host)
        {
            ...
            top.Add(title);
            var small = new Label("점수 = (일차×dayPoints + 게이트×gatePoints) × 자극 × 살아 있는 둥지 비율 · 점수가 곧 포인트");
        ...
        void Render()
        {
            ...
            var g1 = Grid();
            g1.Add(Cell("Day Points (일차당)", w.dayPoints, v => { w.dayPoints = Mathf.Max(0, v); Changed(); }));
            g1.Add(Cell("Gate Points (게이트당·합)", w.gatePoints, v => { w.gatePoints = Mathf.Max(0, v); Changed(); }));
            g1.Add(Cell("강화분 진폭 A", w.stimulusAmplitude, v => { w.stimulusAmplitude = Mathf.Max(0, v); Changed(); Render(); }));
            g1.Add(Cell("강화분 지수 p (≥1)", w.stimulusExponent, v => { w.stimulusExponent = Mathf.Max(1, v); Changed(); Render(); }));
            g1.Add(Cell("강화분 선형 b", w.stimulusLinear, v => { w.stimulusLinear = Mathf.Max(0, v); Changed(); Render(); }, last: true));
            ...
            g0.Add(Cell("낮 길이(초)", clock.dayDuration, v => { clock.dayDuration = Mathf.Max(1, v); Changed(); }));
            g0.Add(Cell("밤 길이(초) — 달이 뜨고 지는 시간", clock.nightDuration, v => { clock.nightDuration = Mathf.Max(1, v); Changed(); }, last: true));
            body.Add(g0);

            body.Add(GroupTitle("밤 진행"));
            var g2 = Grid();
            g2.Add(Cell("둥지 수 최소", w.nestsPerNightMin, v => { w.nestsPerNightMin = Mathf.Max(1, (int)v); Changed(); }));
            g2.Add(Cell("둥지 수 최대 (0=전부)", w.nestsPerNightMax, v => { w.nestsPerNightMax = Mathf.Max(0, (int)v); Changed(); }));
            g2.Add(Cell("목표 밤 길이(초)", w.targetNightLength, v => { w.targetNightLength = Mathf.Max(1, v); Changed(); }));
            g2.Add(Cell("밤당 버스트 수", w.burstsPerNight, v => { w.burstsPerNight = Mathf.Max(1, (int)v); Changed(); }));
            g2.Add(Cell("버스트 퍼짐(m)", w.burstSpread, v => { w.burstSpread = Mathf.Max(0, v); Changed(); }, last: true));
            body.Add(g2);
            ...
            for (int i = 0; i < w.stimulusBuffs.Count; i++)
            {
                ...
                row.Add(pick);
                row.Add(Mini("base", b.baseValue, v => { b.baseValue = v; Changed(); }));
                row.Add(Mini("per", b.perStimulus, v => { b.perStimulus = v; Changed(); }));
                row.Add(Mini("min", b.min, v => { b.min = v; Changed(); }));
                row.Add(Mini("max", b.max, v => { b.max = v; Changed(); }));
            ...
            for (int i = 0; i < w.roster.Count; i++)
            {
                ...
                row.Add(pick);
                row.Add(Mini("cost", r.cost, v => { r.cost = Mathf.Max(0.1f, v); Changed(); }));
                row.Add(Mini("weight", r.weight, v => { r.weight = Mathf.Max(0, v); Changed(); }));
                row.Add(Mini("minDay", r.minDay, v => { r.minDay = Mathf.Max(1, (int)v); Changed(); }));
                row.Add(Mini("minGate", r.minGate, v => { r.minGate = Mathf.Max(0, (int)v); Changed(); }));
            ...
            var g3 = new VisualElement { style = { flexDirection = FlexDirection.Row, alignItems = Align.Center } };
            {
                ...
                g3.Add(pick);
                g3.Add(Mini("group", t.group, v => { t.group = Mathf.Max(1, (int)v); Changed(); }));
                g3.Add(Mini("interval", t.interval, v => { t.interval = Mathf.Max(1, v); Changed(); }));
                g3.Add(Mini("until", t.untilKilledFraction, v => { t.untilKilledFraction = Mathf.Clamp01(v); Changed(); }));
            }
            body.Add(g3);

            // ── 미리보기 — 꺾은선. 둥지 수는 맵 데이터에서(맵마다 선 하나) ──
            var maps = map.maps.Where(m => m.nests.Count > 0).ToList();
            body.Add(GroupTitle("미리보기 — 둥지 파괴가 밤 총량에 미치는 영향 (총량 = 자극 × 살아 있는 비율, 전체 = 1)"));
            if (maps.Count == 0)
                body.Add(new Label("맵 데이터에 둥지가 없습니다 — 맵 탭에서 둥지를 놓으면 그 수 기준으로 그린다") { style = { color = GdEnum.Warn } });
            else
            {
                body.Add(Legend(new[] { new LineChart.Series { name = "살아 있는 둥지의 스폰", color = BarBase }, new LineChart.Series { name = "자극 강화분", color = BarBonus } },
            ...
            body.Add(GroupTitle("미리보기 — 일차별 점수"));
            {
                ...
                body.Add(Legend(list, total > 0 ? $"x = 일차 · 둥지 {total}개({maps[0].id}) 기준 · 가장 싼 몹 {CheapestNote(w)}" : "맵에 둥지가 없어 점수 0"));
                body.Add(chart);
            }
        ...
        static float Score(GWaveRule w, int day, int gate, int living, int total)
        {
            ...
            return (day * w.dayPoints + gate * w.gatePoints) * ((float)living / total + Bonus(w, total - living, total));
        }
        ...
        void Changed() { combat.PushHist(); RenderWarn(); statLabel.text = $"명단 {combat.wave.roster.Count} · 자극 버프 {combat.wave.stimulusBuffs.Count}"; }
        ...
        static VisualElement Cell(string label, float value, Action<float> set, bool last = false)
        {
        ...
        static VisualElement Mini(string label, float value, Action<float> set)
        {
