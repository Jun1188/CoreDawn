...
- [x] 5a-1 스키마 v2 + `SimDatabase` + 컨버터 표 + 파생 id + 효과 value/duration + **제네릭 모듈 편집기** + 마이그레이션 + `SaveMigrations` id 변환
...
- [x] 5a-2 심 모듈화: 정의에서 조립(공장·몬스터·플레이어), 건물 행동 → 모듈, `InventoryModule`·`CrafterModule`(수제작·조합기 통합, `InventoryPanelView.CraftOnce` 제거)·`ResourceDeposit`(광맥 → 엔티티)·`Loot`·`CoreModule`·`TowerBrain`·`NestModule`, 공장을 `Sim/Factory/`로, `IInteractiveBehavior.Interact`는 뷰 등록부로
...
- [x] 5a-3 뷰 카탈로그(id → 프리팹·아이콘) · SO 삭제 · 인벤토리·UI·배치·세이브가 `Def`+id로
  ...
  - [x] 5a-3e-2 편집기 v2 직접 편집(2026-09-03 완료, ①②③): v1 `GameData.json`·`GameDataExporterV2`·python 이관 도구·`id-migration` 퇴역, 편집기가 `data.json`을 읽고 쓴다. 먼저 결정할 것 — 건물 탭의 역할 프리셋(fireMode 드롭다운 등)을 편집기 편의 계층으로 유지하고 저장만 모듈 조합으로 할지(권장), 모듈 목록을 직접 편집할지. 검증 기준: 편집기 v2 저장 결과 == 현 exporter 출력(바이트 동일).
    - [x] 3e-2 ① Raw 탭(2026-09-03, `feature/editor-raw-tab`, 사용자 지시 "모듈 형태로 바로 편집할 수 있는 Raw 편집 탭 … yaml 같은 직관적인 ui"): 기존 GameData 에디터에 `GdRawTab` 추가 — 팩 `data.json`의 **모든 섹션**(entities·items·recipes·effects·guns·tutorial·sounds·materials·sfx·wave·dayCycle, 왼쪽 드롭다운)을 JToken 트리로 직접 편집(스키마 밖 키 보존), 스키마는 섹션별 정의 타입(EntityDef·ItemDef·RecipeDef·EffectSpec·GunDef·TutorialStepDef·WaveRuleDef·DayCycleDef) 리플렉션(필드·기본값·enum·중첩·`string[]`), 모듈 표는 `SimSchema.EntityModules/ItemModules`; `view`는 **실제 클래스**(`Data/ViewDefs.cs`: EntityViewDef·GunViewDef·ItemViewDef·SoundViewDef·MaterialViewDef + ModelRef·PoseDef·SoundUse·ColliderDef·RigDef·AnimDef·KnockbackDef·IconDef·TextureRef)를 스키마로 쓴다 — 처음엔 편집기 힌트로 흉내 냈다가 사용자 지적("view도 class 있는 데이터인데 왜 추론으로 하는거야")으로 런타임이 읽는 클래스를 Data에 두고(`ViewSchema.Entity/Gun/Item/Sound/Material`이 심 직렬화기로 읽어 캐시·검증, `ViewSpec`(JObject 접근자) 퇴역) 조립기·PackAssets·Gun·BuiltinEffects가 그 클래스를 읽는다. view 클래스는 선택 필드가 많아 유령 행 대신 "+ 필드 추가" 드롭다운, `Dictionary<string,T>` 블록은 "키 이름 + 키"; id 참조(item·effect·gun·sound·materials·ammoFilter)와 튜토리얼 조건 type(TutorialConditions.Kinds)은 드롭다운(값이 목록에 없으면 맨 위에 끼움). 실데이터 전수 검증 통과(entities 29·items 30·recipes 24·effects 7·guns 7·tutorial 16·sounds 27·materials 31·sfx 7·wave·dayCycle), json에 있는 키는 파일 순서·스키마만의 필드는 유령 행(+로 생성), 모듈 "- Type" 제목·표 드롭다운 추가·▲▼×, 편집마다 SimSchema 직렬화기로 즉시 검증, 값 편집은 본문 재생성 없이(언두는 포커스 단위), 저장은 data.json 직접(CRLF 유지·무편집 저장 바이트 동일·값 하나 = diff 한 줄). 과도기 가드: 셸 Save의 v1 내보내기가 data.json을 덮어쓰므로 Raw 편집이 있으면 "건너뜀/취소/덮어쓰기"를 묻는다. 남은 것: items(ItemModules)·guns 등 다른 섹션, 역할 프리셋 결정, 내보내기 퇴역.
    - [x] 3e-2 ② GunDef 용도별 묶음(2026-09-03, `feature/editor-raw-tab`, 사용자 "gundata 데이터 구조를 필드들을 용도별로 묶어줘 · 플라즈마 커터 필요 없는 필드 지워"): 평면 30여 키 → `fire{mode,interval,range,pellets,automatic,damageMultiplier}` · `ammo{unlimited,magSize,reloadTime,filter[]}` · `aim{block,zoom}` · `recoil{x,y,z,kickbackZ,kickbackRot}` · `spread{base,max,perShot,recovery}` · `swing{time,windup,alternate,rotation,position}`. 심은 fire·ammo만 읽고 나머지는 뷰. 묶음이 없으면 기본값(반동·탄퍼짐 0, 스윙 없음). `fireRate`(초)는 포탑의 fireRate(발/초)와 단위가 반대라 `interval`로 개명. v1 편집기·GameData.json은 평면 그대로고 내보내기 `GroupGun`이 묶되 **기본값과 같은 키·빈 묶음은 적지 않는다**(무한 탄이면 magSize/reloadTime, 조준 불가면 zoom, base·max 0이면 spread 전체 생략) — 커터의 불필요 필드 제거가 규칙으로 보장되고 Raw 탭엔 유령 행으로 남는다. 소비자 6파일(WeaponModule·Gun·WeaponManager·WeaponController·GameplayHUDView·WeaponTests) 치환. 검증: data.json diff는 guns 섹션 안(+220/−212), 팩 로드 오류 0, 커터 unlimited/swing 0.42/spread 0/zoom 기본 확인, 심 스위트 37/37, FS 16/16.
    - [x] 3e-2 ③ 셸 개편(2026-09-03, `feature/editor-v2-shell`, 사용자 "각 탭들 pr나누지 말고 한번에 작업해"): 정본을 팩 `data.json` 하나로 — 셸(`GdShell`)이 `GdPack.ReadPack`으로 읽어 `GdPack.ToV1`로 폼 탭용 v1 DTO(`GameDataJson.Root`)를 만들고, 저장은 `GdPack.ToPack`(옛 exporter 이식)으로 되돌려 `GdPack.OrderLike`(디스크 키 순서 유지)·`GdPack.Validate`(`SimDatabase.Load`) 후 `WritePack`(CRLF 유지). 검증: 무편집 저장 = git diff 0(JObject·DTO 두 층 왕복 diff 0, 숫자 허용오차; DTO→JObject 는 텍스트 경유 — `JObject.FromObject`가 float 1.2 를 1.2000000476837158 로 푼다). 셸 바 **[UI ⇄ Raw]** 전환: 같은 자리에서 폼 탭↔Raw 섹션(`SectionOfTab`/`TabOfSection`, MonsterBrain→몬스터 탭), 떠날 때 `PullFromRaw`/들어갈 때 `PushToRaw`로 모델 동기화(`uiDirtySinceRaw`). 퇴역: v1 `Assets/Data/Import/GameData.json`·`MapData.json`(폴더째), `GameDataExporterV2`, `PackMaterialHarvester`, `tools/migrate-gamedata-v1-to-v2.py`·`id-migration-v1-v2.json`, 탭들의 guid ObjectField(모델·아이콘·클립·커브 모델)는 팩 파일 고르기(`GdPackAssets.FileRow/IconRow/ModelList`)로. 맵 탭은 팩 `maps/*.json` 직접(`MapImporter.LoadAll/SaveAll`, 사라진 맵 파일 삭제). 건물 탭 포트 격자 편집기 유지(사용자 "건물은 포트 편집이 ui인데"). 잡은 것: 전투 탭이 빠진 반동·탄퍼짐을 v1 기본값(0.5/5…)으로 채워 플라즈마 커터 왕복 diff → 0(=없음); 건물 3D 미리보기는 팩 템플릿(프리팹 아님)이라 `InstantiatePrefabInScene`이 null → `Instantiate`+`AddSingleGO`.
...
- [x] 5a-4 리소스팩: `StreamingAssets/packs/`, 모델(glTFast)·텍스처(`LoadImage`)·팔레트·emission — 건물 → 아이콘 순. 몬스터(애니메이션)는 내장 유지
  ...
  - [x] 5a-4c 리소스팩 로더: `StreamingAssets/packs/<pack>/` 안의 파일(glb·png·ogg)을 런타임에 읽는다 — glTFast(모델·클립), `LoadImage`(텍스처·팔레트·emission), `UnityWebRequestMultimedia`(ogg). view.model/clips/icon이 guid 대신 팩 상대 경로. Animator·오버라이드 컨트롤러 퇴역 → 상태→클립 재생기(몬스터 walk/attack/hit/die, 벨트 블렌드셰이프). 과도기 산출물(`Art/Models/Towers/*Rig.fbx`·`Art/Models/Monsters/*.prefab`·`Conveyor.prefab`·`Storage.fbx`)을 팩 파일로 변환. `ViewCatalogSO`·베이커 퇴역(런타임 로더+캐시). 남은 프리팹(MonsterNest·ResourceNode·DroppedItem·나무) 정리. 아이콘 시트(`iconGuid` + 서브스프라이트 이름)도 팩 png + 좌표표로.
  ...
  - [x] 5a-4c-6 광맥·둥지 프리팹 퇴역(2026-09-01): 광맥 `ResourceNode_IronOre_A.prefab`의 Visual 서브트리 → `deposit_ground.glb`(칸 단위, 루트 배율 (3,1,3) 접음), 둥지 `MonsterNest.prefab`의 Model → `nest_rocks.glb`; 재질 2종(`M_ResourceNode_Ground`·`M_NestCore_Rocks`) 거둠. 콜라이더·레이어는 **데이터**: `view.layer`("Ground"/"Nest"), `view.meshCollider:false`, `view.colliders[{name, layer, box|capsule}]`(칸 단위 — 광맥 Obstacle 상자 0.75×0.3×0.75(상호작용 마스크 = Obstacle·Interactable·Entity)·Visual 상자, 둥지 NestCore 캡슐 r0.1875 h0.5 → `NestView.indestructibleVisuals`) — `BuildingAssembler.AddDataColliders`. 광맥 뷰는 v1 root `deposit{models, view}`로 적고 exporter가 Ore마다 생성하는 `entities/<item>_deposit`에 병합; 둥지는 `Building:Nest`의 `models`/`view`. `AddViewComponents`에 Deposit(`ResourceDepositView`)·Nest(`NestView` + 파괴/불파괴 시각) 추가. `WorldPopulator`: 광맥·둥지도 **마커**(`BuildingAssembler.Marker` + `DressWhenReady`; 굳지 않은 씬은 `DressNow` — 부팅 씬 필수), 스폰 포인트는 이미 맵 사양(`ApplySpawnPoints`)이라 그대로. `World.resourceNodePrefab/nestPrefab` 삭제. 프리팹 파일 자체는 **테스트 씬 4개(HitDidEffect·MainScene·MapPrototype2.0/3.0)가 참조해 남김**(World·런타임은 안 씀). 검증: 마커+Dress 조립 vs 옛 프리팹 바운즈·콜라이더(종류·레이어·크기·중심) 정확 일치, World 재베이크 마커 220(광맥 46·둥지 4·스폰 포인트 8·보스 id 유지), 플레이 220 입힘·DEPOSITS 46·NESTS 4·16/16·오류 0. 교훈: 편집 모드 eval이 예외로 중단되면 인스턴스가 씬에 새어 저장된다 — 저장 전 HEAD와 루트 오브젝트 비교(이번에 `MonsterNest(Clone)`·`nest_rocks` 6개 발견·삭제).
  - [x] 5a-4c-7 시작 잔해 사양을 맵으로 + `DroppedItem.prefab` 삭제(2026-09-01): `MapDataSO.startItems[{itemId, amount}]`(맵 json `startItems[{item:"Item:ShipDebris", amount:15}]`, `MapImporter.ResolveStartItems`), `WorldPopulator.PlaceStartingDrops`가 맵 사양으로 흩뿌린다(여러 종 가능). World의 `StartItem_Debris` 마커 인스턴스 삭제. 바닥 아이템은 `DroppedItem.Build` 코드 조립뿐. `MapImporter`의 중복 cellSize 검사 블록 정리. 검증: 맵 에셋 startItems 1(ship_debris×15), 플레이 DROPS 15, 16/16, 오류 0. **4c 이관 완료 상태**: 런타임이 읽는 뷰 자원은 팩(glb 33·재질 31·아이콘 29·소리 47) + 내장 등록부(Resources/Builtin: missing.png·Effects 17)뿐. 남은 Unity 에셋은 저작 원본/엔진 쪽: `Player.prefab`·`GameUI_UITK.prefab`(엔진), `Art/Models/Towers/*Rig.fbx`·`Buildings/*.fbx`·`Storage.fbx`·`Weapon/PlasmaCutter_Model.prefab`·`Vegetation/*.prefab`(glb 원본 — 건물 5종·벨트·코어는 .blend가 정본), `ResourceNode_IronOre_A/B`·`MonsterNest`·`NestDungeon_Maze`·`GameObject.prefab`(테스트 씬 참조 — World·런타임 미사용), `Art/Materials/**`(재질 값의 저작 원본 — 거두기 도구가 읽음).
  - [x] 5a-4c-8 Blender 축 규약 원인 규명·원본 교정(2026-09-01, 사용자 "원인을 찾는 거야?"): 프로브(−Y 원뿔·+X 상자)를 세 경로로 Unity에 넣어 실측 — glTF(`export_yup`)와 Blender FBX 기본(forward −Z, Apply Transform)은 **같은 매핑**(Blender −Y → Unity +Z, +X → −X); 팀 FBX 프리셋 `unity_out`(`axis_forward='Z'`)만 Y축 180° 다름. 팀 fbx 전부에 Z 180°가 박혀 있었고 `.blend` 원본의 오브젝트 회전은 0 → **원인 = 프리셋의 forward 축**. 게임(포트 East=+X, 벨트 서→동)은 그 fbx 기준으로 맞춰져 있었다. 결정(사용자): 표준 규약으로 통일 — 프리셋 `axis_forward`를 `-Z`로 고치고, 원본 `.blend` 7종(Miner·Smelter·Constructor·Splitter·Merger·Conveyor·Conveyor_Port)을 `tools/blender/rotate_z180_apply.py`로 교정(최상위 오브젝트를 Z 180° 돌린 뒤 **루트에만** Apply Rotation — 자식은 부모 역행렬로 따라가고 로컬 값·애니 키 보존; 자식까지 구우면 Smelter의 애니 자식 Molten/Pot이 어긋남; 숨긴 오브젝트는 잠시 풀어 처리; SpaceShip은 아마추어라 제외). glb 8종을 후처리 없이 재내보내기(`glb_yaw180.py` 패치 폐기 — 남은 용도는 basic.glb 몬스터 한 건: Maya 원본 방향이 옛 프리팹의 손 회전과 반대라 유지, spitter·boss는 원본 그대로). 검증: 바운즈 22/24 카탈로그 일치(미너 rest 포즈·코어 스킨 바운즈만 기존 차이), 벨트 3모양 중심 일치·클립 재생, Boot→World 220·16/16·스냅샷 동일·오류 0. 교훈: Bash 도구에서 Blender에 `/c/...` 경로를 넘기면 `C:\c\...`에 써진다(Windows 경로로 넘길 것); 검증은 항상 실제 팩 파일의 mtime·내용으로.
  - [x] 5a-4c-9 사용자 지적 넷(2026-09-02): ① **아웃라인**: 드롭의 `Outlinable`에 대상 렌더러(`TryAddTarget(new OutlineTarget(sr))`)·스타일(구 프리팹 값)을 넣지 않아 안 그려지던 것 수정. 핑·근접 강조는 대상에 컴포넌트를 붙이던 방식(`EpoOutlines.Ensure`)을 버리고 **`OutlinePool`**(풀의 Outlinable이 대상 렌더러를 가리킴 — EPO OutlineTarget은 렌더러 참조)로: `PingOutlineView`·`MonsterOutlineProximity`가 `Rent/Return`. `EpoOutlines` 삭제. ② **드롭 크기**: `DroppedItem.Build`의 Visual 배율 0.25·위치 y0.5, 콜라이더(0.5@0.1 / 트리거 1.5@0.5) — 구 프리팹 값. ③ **씬 굳히기 완전 퇴역**: World 씬에서 `Spawned` 루트(마커 220) 삭제, `WorldPlaceableBaker`·`ViewMarker`·`PlacedMapObject`·`BakeIntoScene`·`Connect`·`Dress*`·`SpawnOverride` 삭제. 런타임 `WorldPopulator.Populate`가 광맥·둥지·나무·시작 잔해·밤 진입로를 전부 세우고, 코어는 `FactoryBootstrap.AutoPlaceCore`(GameBootstrap이 `map.core` 주입). 에디터는 **`WorldPreviewDrawer`**(Editor, `SceneView.duringSceneGui`)가 맵 배치물을 팩 템플릿의 메시·재질로 `Graphics.DrawMesh` — GameObject 없음(메뉴 Tools/Factory/World preview 토글·reload pack). `MapImporter`는 `WorldPreviewDrawer.Invalidate()`만. ④ **에디터 자동 부팅**: `GameBootstrap.BootAsync`가 팩 미준비 + 플레이어 있는 씬이면 `BootScene.Enter(scene)`로 우회(Boot → 그 씬) — 어느 씬에서 재생해도 팩이 준비된 뒤 시작. 검증: World 직접 재생 → Boot 경유 → 나무 169·둥지 4(보스 유지)·코어 1·광맥 46·건물 174, 스냅샷 PLACED 219·BUILDINGS 174·DEPOSITS 46 동일(스냅샷은 이제 심(Factory)에서 읽음), 16/16, 드롭 Visual 0.25·아웃라인 대상 1, 핑 시 풀 Outlinable이 대상 렌더러 1개를 가리킴, 미리보기 렌더(placements 220·parts 20, 씬 오브젝트 0), 오류 0. World.unity −17.6k줄.
  - [x] 5a-4c-10 뒷정리 ①(2026-09-02, 사용자 지시): 참조 0(guid + **이름** 기준 — 팩 data.json·`Shader.Find`도 확인) 에셋 117파일 삭제. 팀 fbx(Buildings 7종·Towers 리그 6종·Conveyor 폴더 fbx/prefab/controller/anim 12개 — 이제 glb+`.blend`가 정본), PlasmaCutter(프리팹·`PlasmaCutterBlade.asset`·`MeleeWeaponBuilder`·재질 2 — 스크립트 없음, `plasma_cutter.glb`가 원본), URP/Lit 재질 19(Monsters 7·Nest 7·Vegetation 비-Lit 중복 5 — 값은 팩 `materials`에 수확돼 있음), 테스트 씬 4(`MapPrototype3.0`·`TestBefore/MapPrototype2.0`·`UITest`(옛 UGUI, Miner.fbx 소품)·`MainScene`(SEOWORLD 2026-07)), `Prefabs/GameObject.prefab`. **남긴 것**: `Shader Graphs/{FactoryColor,Glass,MoltenColor}.shadergraph`+`Palette.png`(팩이 이름으로 씀 — guid 스캔은 참조 0으로 잘못 봤음), `M_NestCore_Rocks`·`M_ResourceNode_Ground`+`MonsterNest`·`ResourceNode_IronOre_A/B`·`NestDungeon_Maze.prefab`(willer의 `HitDidEffect.unity`가 참조 — 팀원 확인 후), 나무 프리팹 5+`Tree_Bark_Lit`·`Broadleaf_Green_Lit`(PackModelExporter.Trees 원본 — 사용자 결정 대기), `Scenes/Test/MainScene/Global Volume Profile.asset`(World.unity가 씀, 위치만 이상), LowpolySciFiWeaponPack(서드파티 폴더). 검증: recompile 0 에러, 에디트 preload 140/140, Boot→World 건물 174·광맥 46·렌더러 7152·깨진 재질 0·콘솔 오류 0. **미해결(빌드 위험)**: 팩 셰이더 4종(`Shader Graphs/*` 3 + `CoreDawn/Vegetation Lit`)과 `LevelUp/PortFlow`·`LevelUp/DebugVertexColor`가 빌드 콘텐츠 어디에도 guid 참조가 없어 스트리핑됨 → `Resources/Builtin`에 셰이더 참조 에셋을 두고 `PackAssets`가 거기서 찾게 할 것(이름도 `CoreDawn/`으로). glb LOD 없음(glTFast LODGroup 미생성, 원본도 LOD 없었음 — 필요 시 노드 이름 규약+`view.lod`).
  - [x] 5a-4c-11 뒷정리 ②(2026-09-02, 사용자 "HitDidEffect 지워도 돼"): `HitDidEffect.unity`(willer, 2026-08-21)와 그 씬만 참조하던 `MonsterNest`·`ResourceNode_IronOre_A/B`·`NestDungeon_Maze.prefab`, `M_NestCore_Rocks`·`M_ResourceNode_Ground.mat`(빈 `Materials/Nest` 폴더째) 삭제. 새 고아 없음(그 프리팹들이 가리키던 건 GridSystem 재질·Player·InputActions·Volume Profile — 전부 다른 곳이 씀). Palette.png 확인: 팩에 `textures/palette.png`로 이미 내보내져 `factory_color`가 씀; Assets의 원본은 v1 `GameData.json`이 이름(`Palette`)으로 가리키는 내보내기 원본이라 유지. 검증: recompile 0, Boot→World 건물 174·광맥 46·깨진 재질 0·콘솔 오류 0.
  - [x] 5a-4c-12 내장 셰이더 허용 목록(2026-09-02, 사용자 승인): 팩 재질은 런타임에 코드로 만들어져 씬·재질 에셋 어디에도 셰이더 참조가 없었다 → 빌드에서 스트리핑돼 분홍이 될 상황. `BuiltinShaders`(ScriptableObject, `Resources/Builtin/Shaders.asset`)에 셰이더 7종(URP/Lit, `CoreDawn/{FactoryColor,Glass,MoltenColor,Vegetation Lit,PortFlow,DebugVertexColor}`)을 참조로 담아 빌드에 싣고, `PackAssets.MaterialOf`·`PortFlowOverlay`·`FlowFieldDebugView`가 `Shader.Find` 대신 `BuiltinShaders.Of(name)`로 찾는다 — 목록 밖 이름은 오류(팩이 쓸 수 있는 셰이더의 화이트리스트). 셰이더 이름 통일: 셰이더그래프 3종 `m_Path` `Shader Graphs`→`CoreDawn`, `LevelUp/PortFlow`·`LevelUp/DebugVertexColor`→`CoreDawn/…`; v1 `GameData.json` 셰이더 문자열 갱신 후 `GameDataExporterV2.Export()`로 팩 재굽기(data.json 3줄만 변경). 검증: 에디트 preload 140/140에서 `coredawn:material/factory_color`→`CoreDawn/FactoryColor`(팔레트 텍스처 바인딩)·`broadleaf_green`→`CoreDawn/Vegetation Lit`, Boot→World 건물 174·광맥 46·깨진 재질 0·오류 0, **Windows 플레이어 빌드 로그에 7종 전부 `Compiling shader` 확인**(실패 각도: 이 에셋 없이는 팩 셰이더가 빌드 로그에 나타나지 않는다). 주의: `db.Material(id)`는 전체 id(`coredawn:material/…`)로 찾는다 — 검사 eval에서 짧은 id를 써서 잠깐 헛짚었다.
...
### 5. asmdef · 고정 틱 · 세이브
- [x] asmdef 분리(2026-09-02, `feature/asmdef`): 실측(실사용 타입 기준 — 정규식 문자열 집계는 using·주석까지 세서 부풀려짐)으로 계획을 조정. Game↔Presentation 상호 73파일이라 이번엔 안 자르고 **Sim / Game(Data+Game+Presentation) / Editor / Tests** 4개 + 기존 PackExport.Editor. Sim(100) 참조 = Newtonsoft뿐(뷰 참조 실제 0 — 불변식 ①을 컴파일러가 강제), Game(206) = Sim+InputSystem+glTFast+EPO+URP(Core/Universal Runtime)+uGUI, Assembly-CSharp에는 ThirdParty 스크립트 31개만 남음. 선행: 죽은 CoreDawn using 208줄 기계 제거(타입 미등장만, 잘못 지우면 컴파일이 잡는 방향). 함정: 한 어셈블리일 때 통하던 internal 접근(`Def.Id { internal set; }`)이 Tests 분리로 끊김 → `AssemblyInfo.cs`의 `InternalsVisibleTo("CoreDawn.Tests")`(Sim·Game); `UnityEngine.Rendering.Volume`류는 `Unity.RenderPipelines.Core.Runtime` 명시 참조 필요. Core.asmdef·기능별 Contracts는 사용자와 논의 끝에 **보류** — Sim/Game 2개 그림에서는 강제하는 참조가 없어 이득 0(YAGNI); 실제로 어셈블리를 가르는 경계가 생기면(Presentation 분리·모딩·넷코드) 그 경계에만 `X.Contracts`를 만든다. Game/Presentation 분리는 실사용 73파일 — 후속.
- [x] 5a-4d 맵을 팩으로(2026-09-02, `feature/map-pack`, 사용자 "5a4d 먼저 할까"): 맵도 팩 콘텐츠 — `packs/coredawn/maps/<이름>.json`(id `coredawn:map/test_map0`), **MapDataSO·구운 에셋 퇴역**. `MapDef`(Sim, plain — MapTile·StartItemSpec·ResourceNodeSpec·NestSpec·SpawnPointSpec 동반 이주, 멤버 이름은 SO와 동일해 소비자 diff 최소) + `PackMaps`(Managers — 동기 로드·캐시·`Version`(Clear마다 증가), json은 이미 팩 id로 풀린 값이라 검증만·폴백 없음). `World`는 맵 **id 문자열**(`mapId`)만 들고 `Map`을 지연 해석 — 실패는 5초 창으로만 기억(파일이 생기면 자가 회복; 도메인 초기 일시 실패가 영구 래치되던 것을 겪고 고침). `MapImporter`는 삭제가 아니라 **용도 변경**: v1 MapData.json → 팩 maps json 내보내기(id 해석 포함; GdMapTab이 DTO를 공유하므로 이름·ImportAll 유지, 메뉴 'Export maps to pack', GdShell 버튼 '저장 + 맵 내보내기'). `SectionOf`에 Map→map. 소비자 포팅: GridManager·PlacementSystem(Inject 시그니처)·WorldPopulator·WorldMapPanelView·GameBootstrap·WorldPreviewDrawer·WorldTerrainGenerator(에디터 지형 굽기도 MapDef — 그대로 동작). World.unity 리세이브로 SO 참조 한 줄이 mapId 한 줄로. 검증: PackMaps 로드 121×121·광맥 46·둥지 4(보스 id 해석)·나무 169·시작 잔해 1, 미리보기 placements 220, Boot→World 건물 174·광맥 46·깨진 재질 0·16/16·콘솔 0. 남은 것: 타이틀 데이터팩 선택이 생기면 씬 mapId도 팩 선택과 함께 검증할 것.
  - [x] 5a-4e 지형을 커스텀 메시 + 런타임 생성으로(방향 확정 2026-09-02, 사용자): 씬의 `Terrain (Generated)`(절벽 프리팹 6,897개 ≈ 씬 23MB)와 TerrainData·물 메시 에셋(70MB)을 퇴역하고 부팅 때 맵에서 생성한다. 전제 지식: ① Unity Terrain 높이맵은 16bit 고정 — 고저차 2m라 비트깊이는 과잉이지만 못 줄인다. ② 높이맵 2049²는 사용자가 512부터 한 단계씩 올려 실측한 값 — **물가 계단** 때문이며 균일 격자에선 더 못 내린다. ③ 그래서 개편은 균일 격자가 아니라 **등고선 정합 적응 메시**여야 한다: 물가·절벽 등고선(기존 `TraceContours` 마칭 스퀘어 재사용)에 정점을 정확히 올리고 평지는 성기게 — 계단이 해상도와 무관하게 사라진다. ④ 에디터 Build는 60초+ 걸린다 — 런타임 생성은 백그라운드 스레드 + Boot 게이트(Progress)로. ⑤ 조명은 리얼타임이라 굽기 손실 없음; 바위 재질 인스턴싱 꺼져 있어 배칭은 StaticBatchingUtility.Combine 또는 재질 인스턴싱 켜기. ⑥ Terrain이 주던 디테일(풀·꽃) 시스템·스플랫 블렌딩은 대체 필요(풀은 인스턴싱 드로우, 스플랫은 정점색/작은 마스크). TerrainGenSettings·생성 절차 수학(거리장·워핑·Submerge)은 그대로 재사용. 미리보기: 반투명 오버레이를 거쳐 최종적으로 **배치물과 같은 방식**으로 — 런타임 빌더를 미리보기 모드(콜라이더·배칭·풀 생략)로 잠시 세워 메시·재질·행렬만 수확해 DrawMesh(절벽 수천 조각은 재질별 결합, terrainParts 258). 씬뷰가 실제 지형 그대로를 GameObject 0으로 보여준다. **완료(2026-09-02, `feature/terrain-mesh` 커밋 ①~⑥)**: ① TerrainGenSettings 런타임화(Resources/Builtin — 풀·바위·레이어·물 재질이 이 에셋으로 빌드에 실림). ② `TerrainForm`(워핑·미세노이즈 제거 — 높이 = 강 거리장의 해석 함수, 등수위선은 이분법; 런타임 거리장 0.5m 픽셀) + `WorldTerrainBuilder`(청크 8칸: 강·가장자리 64×64 + **물가 정점 스냅**(거리장 기울기로 등수위선에 — 계단 해소의 핵심), 절벽 경계 16×16, 나머지 정점 4개 판; 물·경계벽 포팅). ③ `WorldTerrainCliffs`(경계 걷기 포팅 — 벽 2,073+발치 2,341, 볼록 콜라이더). ④ `WorldTerrainGrass`(DrawMeshInstanced, 좌표 해시 결정적, 카메라 근처 청크 지연 생성 LRU). ⑤ `CoreDawn/Ground` 셰이더(사용자: "정점색은 물가에만") — 잔디↔강바닥 2층을 정점색 R로 블렌드, 절벽 바위색 정점색·16×16 경계 청크 폐기(바위 틈 잔디가 자연), 풀도 절벽 타일에 그대로 심는다(마당 깎기 규칙 제거). 손 URP 셰이더 함정: 이 프로젝트 렌더러는 **디퍼드**(m_RenderingMode 2)라 GBuffer 패스 없는 셰이더는 `UniversalForward`로는 스킵됨 — `UniversalForwardOnly`로 태그해야 그려진다(+DepthOnly·DepthNormals·ShadowCaster 패스, BuiltinShaders 목록 등록). ⑥ `World.Awake`가 세움 + 에디터 메뉴 Terrain preview(DontSave); 씬 구운 지형(−54.5만 줄)·TerrainData/물 에셋(74MB)·WorldTerrainGenerator 삭제. 생성 2.2초(거리장 1.1초 — 필요 시 스레드화). 함정: ⓐ 바위 fbx Read/Write 꺼져 있으면 **플레이 모드에서만** 정점 실측·볼록 콜라이더가 죽는다(에디트 모드는 항상 읽혀서 못 잡음 — Lottellu 3종 켬), ⓑ 비활성화한 정적 오브젝트가 GPU Resident Drawer 캐시로 계속 렌더됨(에디트 모드 스크린샷 검증은 GRD 끄고). 검증: Boot→World 건물 174·광맥 46·badMat 0·콘솔 0·16/16, 인게임 스크린샷(풀·꽃·절벽·물) 확인. **⑦ 후속(2026-09-02~03, 같은 가지)**: 미리보기 순수 데이터화(GameObject 0 — 임시 GO 수확이 hierarchyChanged를 울려 무한 재수확하던 렉 수정, DrawMeshInstanced 149배치) · 잔디 GPU 구동(상주 GraphicsBuffer 182만 포기 27MB + `GrassCull.compute` 거리/해시 감쇠·절두체·절벽 박스 오클루전(인셋 0.45칸, 카메라가 박스 안이면 무시, 절벽 타일 풀은 packed 최상위 비트로 면제) + RenderMeshIndirect; DrawMeshInstanced 1023은 상수버퍼 하드 리밋) · 평지 공유 쿼드 인스턴싱(Ground 셰이더 multi_compile_instancing) · 물 2단 32×32 · 앰비언트 Flat→Trilight(`SkyboxTimeView` 하늘 키 × 지평선/바닥 틴트 파생 — 절벽 벽면 덩어리감; Skybox 모드는 다이내믹 스카이라 UpdateEnvironment 스파이크로 배제) · **Vegetation Lit 이식 오류**(`lerp(tex, gradient)`가 잎 텍스처를 버려 캐노피가 단색 과노출 — 원본 그래프의 생성 HLSL과 대조해 `tex × gradient`로; 원본은 NdotL 없는 Emission 셰이더, 잎 노멀은 구형으로 구워져 잎별 조명은 메시가 안 만든다) · `TerrainGenSettings` 죽은 필드 19개 삭제 + 재료 직접 참조(TerrainLayer→텍스처 2장, Water.mat Resources 퇴거) · 플레이 종료 시 PackAssets/PackMaps 캐시 Clear(스테일 missing 수정) · 죽은 `DayNightLightingView` 삭제. 남은 아이디어: GRD vs StaticBatching A/B, 거리장 스레드화.

- [x] 심 자기 시계 — 고정 20Hz(2026-09-03, `feature/fixed-tick`, 사용자 결정: A(틱)→B(fNBT) 두 PR, 플레이어 무기도 고정 틱에): `SimWorld`(Sim; `Entities`+`TickCount`/`Now`=틱수×0.05+시스템 순서 `SimOrder` Effects→Monsters→Players→Waves→Factory, `Step()`)와 `WorldRunner`(Managers; 프레임 dt 누적 → 정수 스텝, 따라잡기 상한 5, 남은 시간은 `SimWorld.FrameAlpha`). 계획의 `World` 이름은 Game `World`(씬 월드 MonoBehaviour)와 겹쳐 `SimWorld`. `SimHost.Sim`이 루트, `SimHost.World`는 `Sim.Entities`. 시스템은 생성자에서 스스로 등록·Dispose에서 해제(`ISimSystem`) — `MonsterSystem`/`PlayerSystem`/`EffectSystem`/`WaveSystem`/`FactorySystem` 생성자가 `SimWorld`를 받고, `MonsterSystem.Now`·`PlayerSystem.Now`·`FactorySystem.Now`는 전부 `SimWorld.Now`(자기 누적 시계 삭제; `WaveSystem.Now`만 "이 밤 시작 뒤 흐른 시간"이라 유지). 공장은 자기 10Hz 유지 — 월드 스텝 `_stepsPerTick`(=2)마다 `RunTick`(정수 카운트, float 누적 없음), `Advance`/`RestoreClock`/`_maxCatchUpTicks` 퇴역(시계 복원은 `SimWorld.RestoreClock`), `TickLeftover`는 지난 스텝 수+FrameAlpha. `SimRunnerBehaviour`·`FactoryBootstrap.Update` 퇴역. 뷰 보간: `Entity.PrevPosition/PrevFacing`(스텝 시작에 `BeginStep`)을 `MonsterView.SyncTransform`이 `FrameAlpha`로 Lerp/Slerp. 주야(`TimeManager.Cycle.Advance(Time.deltaTime)`)는 Game 층이라 그대로. 테스트는 `new SimWorld()`+`Step(n)`(공장 테스트 `RunSim`은 10Hz 틱 수×2). 검증: 리컴파일 0 · 심 스위트 37/37 · FactoryScenarioTests 16/16 · World 플레이 20Hz(6.9초에 138틱)·예외 0.
- [x] 세이브 = NBT(2026-09-03, `feature/nbt-save`, 사용자 결정: fNbt dll): **fNbt 1.1.1**(BSD-3, netstandard2.0) → `Assets/Plugins/fNbt/`. `SaveNbt`(Game/Save): DTO↔`NbtCompound` 리플렉션 매퍼 — public 필드, `[JsonProperty]` 이름 그대로(파일 키가 JSON 시절과 같다), null·`[JsonIgnore]`·const 생략, bool→Byte·int→Int·uint/long→Long·float→Float·double→Double·enum→String·Vector3/Quaternion→Float 목록·Vector2Int→IntArray·List→List(빈 목록도 원소 타입 지정 — fNbt 는 타입 없는 빈 목록을 안 쓴다)·Dictionary/클래스→Compound·NbtTag 통과. 읽기는 숫자 폭 관용·없는 태그는 초기값. `SaveFile.Modules`는 `Dictionary<string, NbtCompound>`, 본체 `save.nbt.gz`(root "coredawn"{schemaVersion, meta, modules}, `SaveStorage.ToRoot/FromRoot`, gzip 은 fNbt), 요약 `meta.json`은 JSON 유지(`SaveJson` 은 그 용도만). `ISaveModule.Restore(NbtCompound)`, 모듈 7개는 `SaveNbt.FromTag<Dto>`. Sim 의 `ISaveableModule`(JToken)은 그대로 — 경계에서 `SaveNbt.ToJson` 다리(Sim 은 fNbt 를 모른다; Sim 상태엔 좌표 없음). 스키마 **v6**, `SaveMigrations.OldestReadable = 6`: v1~v5 JSON 단계 넷 삭제, 옛 `save.json.gz` 슬롯은 `SaveStorage.Read` 가 오류 로그 후 null. `SaveManager.DebugRoundTripDiff` 는 `SaveNbt.DeepEquals`. 검증: `SaveNbtTests` 7/7(왕복·생략·숫자 폭·JSON 다리·파일 압축/비압축·옛 슬롯 거절·DeepEquals) · 기존 스위트 37/37 · 플레이: 제자리 왕복 일치 7/7, 슬롯 저장→Boot 경유 불러오기→재저장(v6, 건물 174/174, progress·world·combat·player 동일, time·tutorial·factory 는 시간 진행분만 차이) · 오류 0.
...
- 4c 이관 완료(5a-4c-1~7). 다음: asmdef → 3e-2(편집기 v2 직접 편집 — 재질·모델 배열·아이콘 좌표표 UI 포함) → 고정 틱·SharpNBT.

### 2026-09-02~03 — 5a-4e 지형 런타임화 + 잔디 GPU + 조명·셰이더 후속 (`feature/terrain-mesh`, 커밋 13개)
- 사용자 설계 그대로: 도메인 워핑 제거, 메시는 "1×1 쿼드 하나 + 64×64 하나", 높이차는 물가에만(정점을 등수위선에 스냅), 절벽은 height 없음(벽은 프리팹), 정점색은 물가만("바위 틈이라고 잔디가 안 자라는 건 아니잖아"), 잔디는 셰이더(GPU) 구동 + 컴퓨트 컬링(절두체·거리·심플 절벽 박스 오클루전), 물 32×32, 미리보기는 "entity랑 똑같은 방식"(GameObject 0).
- 결과: World.unity −23MB(−54.5만 줄), 지형 에셋 −74MB, 부팅 생성 ~2.6초(거리장 1.0초 + 잔디 0.8초), 잔디 182만 포기 27MB GPU 상주. 물가 계단은 정점 스냅으로 해상도 무관 해소(사용자가 512부터 실측해 2049²까지 올렸던 문제).
- 함정 기록: ⓐ 디퍼드 렌더러에서 GBuffer 없는 손 셰이더는 `UniversalForwardOnly`로 태그해야 그려짐 ⓑ 바위 fbx Read/Write는 플레이에서만 터짐 ⓒ GRD(BRG)가 비활성 오브젝트를 계속 렌더(캡처는 GRD 끄고) ⓓ 컬링은 직전 프레임 카메라 기준(이동 후 한 프레임 띄우고 캡처) ⓔ 수동 SceneView `Camera.Render()`는 URP에서 불신뢰 — 플레이 카메라만 ⓕ 플레이 종료 후 PackAssets는 IsReady=true인데 캐시는 시체 → EnteredEditMode에서 Clear ⓖ 조명 A/B는 플레이가 일출에서 시작하므로 `DayCycle.RestoreState`로 정오 고정 후 같은 세션에서 전 조건 캡처.
- 사용자 지적: "말없이 편집하지 말고 말 좀 해봐"(편집 전 근거·계획을 출력으로), "원본 프리팹은 멀쩡한데 네가 셰이더를 잘못 만든 게 아닐까"(→ 이식본을 먼저 의심, 생성 HLSL 대조), "GameObject가 왜 필요한데?"(→ 미리보기 전면 데이터화).
- 검증: 리컴파일 0 · 심 스위트 37/37 · FactoryScenarioTests 16/16 · Boot→World 지형 정상(지면 Grass_Albedo/Sand_Albedo, 물 WaterVolume-URP) · 정오 고정 캡처(잎 결·Trilight 절벽).
- 다음: 3e-2 편집기 v2 직접 편집 → 고정 틱(`World.Now` 20Hz)·fNBT 세이브.

### 2026-09-03 — 3e-2 ③ 편집기 셸 개편: 정본 = 팩 data.json (`feature/editor-v2-shell`)
- 사용자 결정: 폼 탭은 그대로 두고 저장·불러오기만 팩으로(`GdPack.ToV1/ToPack` 양방향), 같은 자리 **[UI ⇄ Raw]** 버튼으로 전환("그냥 같은 위치에 둬서 [ui/raw] 전환 버튼 추가하는 방향은?"), 건물 탭 포트 격자 유지("건물은 포트 편집이 ui인데"), 탭별 PR 나누지 않고 한 번에.
- 왕복 검증에서 잡은 것 셋: ① 전투 탭이 빠진 반동·탄퍼짐(-1)을 v1 기본값으로 채워 플라즈마 커터에 spread 가 생김 → 0(=없음) ② `JObject.FromObject` 가 float 1.2 를 1.2000000476837158 로 풀어 muzzleHeight 4곳 diff → 텍스트 직렬화 경유 ③ 폼→팩 변환이 view 키 순서를 바꿔 160줄 diff → `GdPack.OrderLike` 로 디스크 순서 유지. 세 가지 모두 잡은 뒤 무편집 저장 = git diff 0.
- 함정 기록: ⓐ 건물 3D 미리보기는 직전 플레이가 채운 `PackAssets` 정적 캐시 덕에 우연히 보였다 — 플레이 종료는 도메인을 안 비우므로 "새 도메인(리컴파일 직후)에서" 확인해야 한다 → `PackAssets.TryModelOf/LoadModelAsync` 로 편집 모드 비동기 로드 ⓑ `PreviewRenderUtility.InstantiatePrefabInScene` 은 프리팹이 아닌 팩 템플릿에 null ⓒ `unity get_console_logs` 는 최근 100건만 준다 — 긴 플레이 보고는 `%LOCALAPPDATA%/Unity/Editor/Editor.log` 에서 grep.
- 검증: 리컴파일 0 · JObject·DTO 왕복 diff 0 · 무편집 저장 git diff 0(data.json·maps) · 심 스위트 37/37 · FactoryScenarioTests 16/16 · 전환 동기화(건물→Raw 같은 항목, Raw 총/몬스터/아이템/소리/튜토리얼→해당 폼 탭 같은 항목) 리플렉션 확인 · 새 도메인 건물 미리보기 캡처.
- 다음: 고정 틱(`World.Now` 20Hz)·fNBT 세이브.

### 2026-09-03 — 5단계 고정 틱 (`feature/fixed-tick`)
- 사용자 결정(질문 3개): A 고정 틱 → B fNBT 세이브 두 PR · NBT 는 fNbt dll 도입 · 플레이어 무기도 고정 틱(발사 위상 ≤50ms 양자화 감수).
...
### 2026-09-03 — 5단계 fNbt 세이브 (`feature/nbt-save`)
- fNbt 함정: ① 원소 없는 `NbtList`는 `ListType`이 없으면 저장 시 `NbtFormatException` — 컬렉션의 제네릭 원소 타입에서 미리 정한다(`SaveNbt.ListTypeOf`). ② 태그는 부모가 하나뿐이라 SaveFile↔루트 사이는 `Clone()`. ③ uint 를 Int 로 넣으면 JSON 다리에서 음수 → Long 으로.
...
### 2026-09-04 밤 — 몹·건설·아이템 묶음 (`feature/night-batch`, 사용자 "am8:00까지 다 끝내놔, push하지 말고")
- **체력바가 몬스터 자기 몸에 가림** → `WorldHealthBar.OverlayMaterial`: UI/Default 복제에 `unity_GUIZTestMode = Always`(캔버스가 쓰는 재질 속성이라 셰이더 추가 없음). 바 이미지 전부 이 재질.
- **콜라이더 vs 플로우필드 간격 중복?** → 없음. 겹침 해소는 심 `MonsterSystem.SolveCrowd`(CrowdRadius, 위치 보정) 하나뿐이고 뷰 캡슐은 kinematic 이라 몬스터끼리 물리로 밀지 않는다(플레이어 접촉·레이캐스트용). 비용 필드엔 반지름 팽창(clearance)이 없고, 이동(`TickDirection`)과 겹침 해소가 각자 `IsWalkable` 로 셀 진입만 막는다 — 같은 규칙을 다른 변위에 적용하는 것이지 간격 계산 중복은 아니다.
- **나무를 부수고 지나감** → 원인 둘. ① `GridManager.RefreshCostsIn` 호출처가 **전무**(5a-2f 에서 BuildingEntity 훅이 사라지며 끊김) — 워커용 CostField 가 시작 뒤 놓인 건물·나무를 몰라 플로우필드가 직진하고, 몬스터는 걷다 막힌 것을 `FindBreachTarget` 으로 부쉈다. → `GridManager.SubscribeFactory`(Placed/Removed → `RefreshCostsIn` + `FlowFieldManager.MarkDirty`), `Start` 에서 `simGridSystem` 이 생긴 뒤 전체 재굽기(그 전 Bake 는 `node.gridCoord` 폴백으로 건물을 못 찾았다). ② 나무가 플레이어 벽과 같은 "HP 비례 뚫는 길"(120HP → +60)이라 숲을 돌아가는 것보다 쌌다 → 처음엔 중립 팩션 하드코딩(Blocked), 다음엔 `breachable` 필드를 넣었다가 사용자 지적("이상한거 넣지 말고 시드 올려")으로 둘 다 걷어냄. 최종: **통과 비용 = 기본 + 현재 HP 비례(상한 200) + `threatSeedCost`(합산, 사용자 결정)**, 나무 시드 80→1000(팩 데이터). HP 는 정의 maxHp 가 아니라 살아 있는 엔티티의 **현재** HP(사용자 "현재 hp 기준으로 해야") — 피격·수리마다 `HealthModule.OnHealthChanged` 훅으로 그 건물 칸만 다시 칠하고 `MarkDirty`(`GridManager.HookHealth`). 정의값으로 읽던 때 나무 HP 항이 0으로 나오던 이유는 미확인 — 현재 HP 로 바꾸면서 값이 맞는지 플레이에서 확인. 사실 `threatSeedCost` 는 그전엔 목표 시드에만 쓰여 나무엔 효과가 없었다 — 그래서 시드만 올려서는 안 풀렸던 것. 유한 비용이라 갇히면 여전히 뚫는다.
- **건물 자동 방향** → `PlacementSystem.TryAutoRotation`: 칸이 바뀔 때 후보 회전 4개의 포트가 인접 건물 포트와 맞물리는 수(`HasPortAt`, 입↔출 마주 봄)를 세고, 카메라가 보는 축(`LookDir`)의 이웃이면 +10. 동점은 현재 회전 유지. 손으로 돌리면 그 칸에서는 손 회전 유지(`manualRotationCell`), 칸을 옮기면 다시 자동. 헤드리스: 벨트 (62,61)(63,61) 뒤 (64,61) 2→0, 앞 (61,61) 1→0, 이웃 없음 → 유지.
- **설치 칸 표시(스타크래프트식)** → `DrawCellOverlay`: 풋프린트 칸마다 맵 타일·점유·높이를 따로 판정해 초록/빨강 사각형(`Graphics.DrawMesh`, UI/Default ZTest Always 반투명 — 풀 위에 보이게). 프리뷰 색은 전체 판정 하나라 어느 칸이 문제인지 못 보여 주던 것.
- **아이템 눕히고 두께** → `ItemSlabMesh`: 스프라이트 타이트 메시(알파 실루엣)를 위/아래 면으로, 실루엣 경계 변마다 옆면(서브메시 1, 단색). 재질은 URP Lit 알파클립(시트 텍스처) + 옆면 단색. `BeltItemView.lieFlat`(기본 켬, 두께 0.06m, 벨트 상판 위 0.56m, 아이콘 위쪽이 흐르는 방향), `DroppedItem` 도 슬랩(0.25 스케일, 두께 0.05m, 바닥 위 0.25m 회전).
- 검증 방법 메모: 배치 모드 프리뷰 루프는 헤드리스에서 안 돌아(조준 레이가 땅을 못 잡음) `RenderPipelineManager.beginCameraRendering` 에 `DrawCellOverlay` 를 걸고 임시 카메라로 캡처. 벨트 아이템은 `BeltSegment.TryAddItem/AddItemAt`, 바닥 아이템은 `PlacementBridge.DropAt`.

### 2026-09-04 낮 — 같은 브랜치에 추가: 비용 스냅샷 · 보스 스케일 · (건설 소모 미재현)
- **비용 필드 경합** — 워커(`FlowField.Rebuild`, A* `PathFinder`)가 `GridManager.Costs`의 살아 있는 배열을 그대로 읽어, 메인이 칸 단위로 고치는 동안(설치·철거·피격) 한 번의 탐색 안에서 옛/새 값이 섞일 수 있었다(자기 치유 ≤2초, 실해는 작음). 사용자 제안(NativeArray 포인터 swap)은 증분 갱신 때문에 변경 목록 재생이 필요해 복잡도만 늘고, 실측(121×121 맵 4등분 234,256칸 int+bool 1.17MB) `Array.Copy` 0.050ms·`NativeArray.CopyFrom` 0.069ms 로 복사가 비용이 아니다 → `CostField.CopyFrom` + 소비자(`FlowFieldManager.costSnapshot`, `PathRequestQueue.costSnapshot`)가 `Version` 이 다를 때만 복사. 검증: 강제 재계산 뒤 snapVer==gridVer(518), 배열 참조 분리, hasField, 표본 2669칸 중 872칸 next 보유(낮 필드).
- **보스 스케일** — 둥지(`NestView.SpawnBossAtPoint`/`RestoreBoss`)가 보스를 자기 `transform` 아래에 붙였는데, 건물 뷰 루트는 `BuildingAssembler` 가 칸 크기(4)로 스케일해 둔 것이라 보스 몸·캡슐이 4배(캡슐 16m, 몸 12m)였다 → `NestView.MonsterParent`(= `transform.parent`, 스케일 1인 Spawned 컨테이너)로 스폰. `MonsterAssembler.Build` 는 스케일된 부모를 받으면 LogError(조용한 보정 없음). 검증: 둥지 보스 4마리 lossy 1.00·캡슐 4.0m·부모 Spawned, 첫 틱 뒤 y=2.0(접지), 캡처 boss3.png.
- **"건설 시 소모 안 됨"** — 사용자 정정: 실은 **시작 아이템 이중 지급**(가방 뒤 칸부터 빠져 핫바가 안 줄어 오해). 원인: 에디터에서 World 를 바로 재생하면 `GameBootstrap.BootAsync` 가 "팩 자원이 없어 Boot 를 거쳐 다시 엽니다"로 World 를 두 번 여는데, `SimHost`/`SimRunner` 는 씬을 넘어 살아 `PlayerSystem.Spawn` 이 첫 번째 플레이어 엔티티를 재사용하고, `PlayerInventoryHolder.Awake` 는 매번 `SeedStartingItems` 를 돌려 같은 가방에 두 번 넣었다(철판 50→100). → 홀더가 `SimRunner.Players.Entity` 가 없거나 제거된 경우(새 엔티티)에만 시딩, 재사용이면 Log 로 알린다. 검증: 첫 진입 철판 50, Boot 경유 강제 재진입 뒤에도 50(재사용 로그 1건).
- 부수 발견: `NestView.SnapBossToGround` 는 뷰 transform 을 옮기지만 `MonsterView.SyncTransform` 이 매 프레임 심 위치로 덮어써 사실상 무효(접지는 `MovementModule.StickToGround` 가 첫 틱에 함). 코드는 건드리지 않음 — 별도 정리 후보.
- **씬 재로드 뒤 유령 광맥 → 심 소유권 정리** — 실측: Boot 경유 재진입 뒤 심 엔티티 229→275, 광맥만 두 배(구리 14→28·수정 10→20·철 22→44). 1차로 `FactoryBootstrap.OnDestroy` 에 광맥 제거를 더했다가, 사용자 지적("Entity 소유는 Sim 이지 View 가 아니다")으로 방향을 바꿨다: 뷰·부트스트랩이 `OnDestroy` 에서 심 엔티티를 빼던 역방향 코드(`FactoryBootstrap` 건물 루프, `NestView.OnDestroy`, `PlayerView.OnDestroy`)를 지우고, 씬 전환 게이트 `BootScene.Start` 가 `SimRunner.Reset()`(전투 시스템 4개 Dispose, 공개로 승격) + `SimHost.Reset()` 으로 옛 월드를 통째로 버린다. 게임 안의 Single 로드는 전부 Boot 를 지난다(불러오기·새 게임·에디터 직접 재생 라운드트립; 타이틀 복귀는 심 소비자가 없어 다음 Boot 까지 방치). 뷰에 IDisposable 은 필요 없다 — 심→뷰 방향은 이미 `Entity.Removed` 구독으로 뷰가 스스로 사라진다. 검증: 첫 진입·Boot 재진입·저장→불러오기 모두 229(광맥 46, 철판 50), 홀더 재사용 로그 0, 스위트 6개·FactoryScenario 16 통과.
- **몬스터 사망 → 제거도 심이** — 남아 있던 마지막 역방향(`view.deathDelay` → `EntityView.Invoke(FinishDeath)` → 뷰 파괴 → `MonsterView.OnDestroy` 가 `Monsters.Despawn`)을 뒤집었다. 시체 시간은 심 정의 `MonsterBrain.corpseSeconds`(팩: basic·spitter 2.0, boss 4.5 — 옛 `view.deathDelay` 값을 옮기고 뷰 필드는 삭제, `Resolve` 가 음수 거부), `DeadState` 가 세어 `MonsterSystem.Despawn`. `MonsterBrainModule.Tick` 이 죽으면 바로 돌아가던 것을 죽었을 땐 `DeadState` 만 틱하게 고쳤다. 뷰는 `MonsterView.HandleDeath` 를 비워 연출만 틀고 `Entity.Removed` 로 사라진다. 편집기 폼은 v1 DTO(`GameDataJson.MonsterDto`) 왕복이라 `GdPack`·`GdCombatTab`·`GdMonsterTab` 목록에 `corpseSeconds` 를 더해 편집 저장에서 필드가 떨어지지 않게 했다. 검증: 보스 Kill 뒤 2.3초엔 시체(뷰 1·심 dead 1), 9초 뒤 둘 다 없음; 일반몹 2초 뒤 제거; 팩 strict 오류 0.
- **몬스터 뷰 풀(`MonsterViewPool`)** — 사용자 요청. 밤 버스트(포인트 100+50×일차, basic 10, 5회 분할)가 한 프레임에 열 마리 안팎을 조립하고 스킨 메시 Instantiate·월드 캔버스 생성이 그 스파이크라 정의 id 별로 "완성된 뷰 한 벌"을 보관한다. 심이 엔티티를 지우면 `MonsterView.OnEntityRemoved` 가 `Detach()`(새 훅 `EntityView.OnEntityDetached` 로 두뇌 구독 해제) 뒤 `Return`, `MonsterSpawner.AttachView` 가 `Rent` 로 꺼내 `AttachEntity`. 되돌리는 상태: 위치·부모(풀), 심 참조(뷰), 사망 플래그·클립·가라앉기(`MonsterVisualController.OnEnable` — 이미 풀링을 예상하고 있었다), 체력바(뷰 릴레이라 자동; `HealthBarUI.OnEnable` 이 엔티티 없는 뷰에서 `Health` 를 읽던 NRE 만 가드). 풀 루트는 씬 오브젝트라 씬 전환에 함께 사라지고 남은 참조는 꺼낼 때 null 검사. 검증: 같은 GameObject 3회 재사용(인스턴스 id 동일), 재사용 뷰가 Idle 클립·체력바 표시·두뇌 연결·PivotToBottom 0.6, 보스도 재사용(캡슐 4m·lossy 1), 예외 0.
- **정리(2026-09-04, 사용자 "손대지 않은 것이랑 계획서 정리")** — `NestView.SnapBossToGround` 삭제(뷰 transform 을 옮겨도 `SyncTransform` 이 심 위치로 덮어써 무효, 접지는 `MovementModule.StickToGround`). `World.unity` 의 죽은 `startingItems` 오버라이드 4개 제거(빈 7번째 항목, 죽은 SO guid). 편집기 몬스터 폼: v1→v2 두뇌 키 목록을 `MonsterBrainModuleDef` 의 JsonProperty 반사(`GdPack.MonsterBrainKeys`)로, 폼이 모르는 키는 `unknownJson` 으로 왕복(`ExportMonster`), 폼에 없는 정의 키는 로드 때 한 번 경고. 계획 상위 체크박스 5a-1~5a-4·5a-4c 를 완료로(하위가 전부 끝남; 5a-0 은 착수 기록이 없어 그대로).
