...
- [ ] 5a-2 심 모듈화: 정의에서 조립(공장·몬스터·플레이어), 건물 행동 → 모듈, `InventoryModule`·`CrafterModule`(수제작·조합기 통합, `InventoryPanelView.CraftOnce` 제거)·`ResourceDeposit`(광맥 → 엔티티)·`Loot`·`CoreModule`·`TowerBrain`·`NestModule`, 공장을 `Sim/Factory/`로, `IInteractiveBehavior.Interact`는 뷰 등록부로
  ...
  - [x] 5a-2e-3 둥지를 심으로(2026-08-31, `feature/nest-module`): `NestSpawner` → **`Nest`**(`NestModuleDef{bossRecoveryDays, nestRecoveryDays}` → `NestModule`): 스폰 포인트의 심 상태(`NestPoint{position, hasBoss, boss, isDestroyed, destroyedDay}`), 둥지의 파괴/복구(날짜 기반, 밤에 `Health.ResetToFull`), **무적 규칙을 `IDamageInterceptor`로**(스폰 포인트가 하나라도 살아 있으면 0 — `DamageGateModule` 삭제), **보스 사망은 엔티티 `Died` 이벤트로**(뷰 폴링 삭제), 자리 복구는 낮에(`OnDayStarted`), 옛 규칙(교전 구역 없음) 둥지의 밤 보스 보충(`RefillBossesAtNight`). 보스를 **세우는** 것은 아직 뷰(프리팹이 뷰 에셋) — 모듈이 `BossNeeded(i)`로 부르고 뷰가 `MonsterSpawner`로 세워 `BindBoss`. 낮 방어 스폰의 시점·자리(플레이어 거리·화면 가림 = PhysX 레이캐스트)는 뷰에 남음. 주야는 뷰가 `OnDayStarted/OnNightStarted(day)`로 전달(`Day` 브리지). `NestView`는 자리(Transform)·보스 프리팹·외형·주야 전달·낮 스폰 판정만; `SyncModule`이 포인트·복구일을 심에 민다(WorldPopulator 배치·프리팹 둘 다). 세이브(`CombatSaveModule`)는 자리 상태를 모듈에서 읽고 같은 표면으로 복원. 테스트 `NestTests` 6개(무적·보스 사망→자리 파괴·전멸→피해·둥지 복구·자리 복구+보스 호출·밤 보충·복원). 검증: World 둥지 4 모듈·무적, 보스 처치 → 자리 파괴·피해 통과(500→450), 세이브 왕복(보스 15)·밤 웨이브·콘솔 0
