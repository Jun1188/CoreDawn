...
- [ ] 5a-4d 맵을 팩으로: MapDataSO 퇴역 — 맵도 팩 콘텐츠(`maps/*.json`), `World`는 SO 참조 대신 맵 id, startItems·nests 포함. 소비자: World.map(씬 바인딩)·GridManager/PlacementSystem.Inject·WorldMapPanelView(미니맵)·에디터 지형 굽기(WorldTerrainGenerator — 그때도 json 직접 읽으면 됨). Boot의 데이터팩 선택·세이브 맵 id와 같이 움직일 것.
