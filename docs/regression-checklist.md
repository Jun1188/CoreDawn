...
## 5. 2026-09-04 밤 작업으로 추가된 확인 항목

- 몬스터 체력바가 몬스터 자기 머리·몸에 가려지지 않는다(낮은 각도에서 봐도 바가 위에).
- 나무가 있는 곳: 몬스터가 나무를 치지 않고 **돌아간다**. 나무 칸의 `GridManager.Costs.WalkableAt == false`, `Costs.CostAt(cell) ≈ 10 + 현재HP×0.5 + threatSeedCost(1000)`. 우회가 그보다 비쌀 때(숲에 갇힘)만 나무를 부순다. 건물을 때리면 그 칸 비용이 내려간다(현재 HP 기준).
- 건물을 놓거나 부순 뒤 몇 초 안에 몬스터 경로가 바뀐다(비용 배열 갱신 + 플로우필드 재계산).
- 벨트 옆 칸에 벨트를 대면 방향이 자동으로 이어진다. 손으로 돌린 뒤 같은 칸에서는 손 회전 유지, 칸을 옮기면 다시 자동.
- 배치 프리뷰 아래 칸마다 초록/빨강 사각형이 풀 위로 보인다(점유·강·절벽·높이차).
- 벨트 위 아이템이 두께 있는 판으로 눕고 흐르는 방향으로 놓인다. 바닥 아이템도 판으로 돈다. 조준(집기)은 그대로 된다.
- 둥지 보스가 일반몹의 3배 정도 크기(몸 약 3m)로 땅 위에 선다. 캡슐이 16m 로 커지거나 몸이 땅에 묻히지 않는다. 콘솔에 `[MonsterAssembler] … 부모 … 스케일` 오류가 없다.
- 밤에 건물을 놓거나 부순 직후에도 몬스터 경로가 끊기지 않는다(워커는 비용 스냅샷을 읽는다).
- 에디터에서 World 를 바로 재생해도(Boot 경유 재진입) 시작 아이템이 한 번만 들어간다(철판 50·기어 50). 콘솔에 `[PlayerInventoryHolder] … 재사용` 로그가 있어도 수량은 그대로.
- 불러오기·새 게임·Boot 경유 재진입 뒤 심 엔티티 수가 첫 진입과 같다(test_map0: 229, 광맥 46). 콘솔에 `[PlayerInventoryHolder] … 재사용` 로그가 없다.
- 몬스터를 죽이면 시체가 corpseSeconds(일반 2초·보스 4.5초) 남았다가 심이 지우고 뷰가 따라 사라진다. 시체가 영원히 남거나 즉시 사라지지 않는다.
- 몬스터가 죽어 사라진 뒤 같은 종이 다시 나올 때 죽은 자세·가라앉은 위치·꺼진 체력바가 남지 않는다(뷰 풀 재사용). 계층의 MonsterViewPool 아래에는 비활성 뷰만 있다.

## 6. 타이틀 화면 (2026-09-07, `feature/title-screen`)

빌드 0번 = Title(Boot 씬은 타이틀에 합침, 2026-09-07). Title 을 재생하면 로딩 상자(PREPARING LAUNCH SEQUENCE·퍼센트·기울어진 바·지금 읽는 파일명)가 팩 자원 140개를 세고, LAUNCH SEQUENCE READY / ALL SYSTEMS GO 를 잠깐 보인 뒤 걷히며 메뉴가 글리치로 생겨난다 — 로딩은 한 번.
1. **로딩 → 메인** — 로딩 상자가 수 초 안에 걷히고 벨트·정육면체·메뉴 3개·점선 와이어 3개(레티클 포함)가 보인다. `[TitleBeltScene]` 오류 0.
2. **게임 시작** — 앞 아이템이 분배기에서 오른쪽 분기 벨트(4칸)로 빠지고 우주선이 착륙, 서브 메뉴(새 게임·불러오기·뒤로가기)가 글리치로 등장. 와이어는 우주선 위 앵커 3개로.
...
6. **새 게임** — 이륙 클립 → 5초 뒤 2초 페이드 → "WARPING TO STAR SYSTEM" 오버레이(AppFlow)가 PACK → UNLOAD → SCENE WORLD → TERRAIN FORM/TERRAIN(%가 움직인다)/WATER/CLIFFS/GRASS/BATCH → SYSTEMS 100% 를 거쳐 완료 문구 없이 0.6초 페이드 아웃 → World, `[게임 시작] 1일차 낮`. 화면이 서지 않는다.
