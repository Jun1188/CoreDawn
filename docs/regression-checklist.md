...
빌드 순서는 Boot(0) → Title(1). Boot 씬을 재생하면 로딩 화면(LOADING·퍼센트·기울어진 바·지금 읽는 파일명)이 140개 항목을 세고 READY 뒤 Title 로 간다.
Title 을 바로 재생하면 타이틀 자체 로딩 상자(모델 5개)가 뜬다.
...
6. **새 게임** — 이륙 클립 → 5초 뒤 2초 페이드 → Boot → World, `[게임 시작] 1일차 낮`.
